defmodule TelemetryMetricsOTLP.DefinitionTest do
  ...
  describe "compile!/2" do
    ...
    test "stores configured distribution bounds unchanged" do
      integer_above_float_precision = 9_007_199_254_740_993
      bounds = [-10, 0, 2.5, integer_above_float_precision]
    ...
    test "rejects non-list and non-numeric distribution bounds" do
      for bounds <- [[0, :infinity], [:negative_infinity, 0], :not_a_list] do
        metric =
          distribution("http.request.duration",
            reporter_options: [buckets: bounds]
          )
    ...
    test "rejects duplicate and descending distribution bounds" do
      for bounds <- [[1, 1], [1, 1.0], [2, 1]] do
        metric =
          distribution("http.request.duration",
            reporter_options: [buckets: bounds]
          )
